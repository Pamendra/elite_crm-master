  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      BlocProvider.of<AddreportBloc>(context).add(
          onPressedButtonEvent(leadId: widget.id.toString(), name: widget.name.toString(), cname: widget.cname.toString(), gmanager: widget.gmanager.toString(), pmanager: widget.pmanager.toString(),
              services: widget.services.toString(), category: widget.category.toString(), status: widget.status.toString(),
              vdate: widget.date, dealerIds: customerList.map((customer) => customer['id']).toList().toString().replaceAll("[","").replaceAll("]",""),
              gnote: widget.Greport.toString(),
              dealernotes: customerList.map((customer) => { customer['id']: customer['report'] }).toList().toString().replaceAll("[","").replaceAll("]","") ));
            },
              child: Container(
                padding: const EdgeInsets.all(15),
                width:50.w,
        ...
        body: BlocConsumer<AddreportBloc,AddReportState>(
          ...
          Dialogs.showValidationMessage(context, 'Added Report Successfully');
